...
class Gameplay(commands.Cog):
    ...
    async def roll(self, ctx: commands.Context, *, tier: str = None):
        ...
        if iufi.is_valentines_day():
            await func.reduce_partner_cooldown(ctx.author.id, user["couple_id"], "roll")
    ...
    async def quiz(self, ctx: commands.Context):
        ...
        if (retry := user.get("cooldown", {}).setdefault("quiz_game", 0)) > time.time():
            ...
            view.response = await ctx.reply(f"{ctx.author.mention} your quiz is <t:{round(retry)}:R>. If you’d like to bypass this cooldown, you can do so by paying `{iufi.get_main_currency_emoji()} {price}` {iufi.get_main_currency_name()}.", delete_after=20, view=view)
            return 
        ...
        if iufi.is_valentines_day():
            await func.reduce_partner_cooldown(ctx.author.id, user["couple_id"], "quiz_game")
