    ...
    async def quiz(self, ctx: commands.Context):
        ...
        **Examples:**
        ...
        if (retry := user.get("cooldown", {}).setdefault("quiz_game", 0)) > time.time():
            ...
            view.response = await ctx.reply(f"{ctx.author.mention} your quiz is <t:{round(retry)}:R>. If you’d like to bypass this cooldown, you can do so by paying `❄️ {price}` candies.", delete_after=20, view=view)
            return 
