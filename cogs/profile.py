...
WEEKLY_REWARDS: list[tuple[str, str, int]] = [
    (f'{iufi.get_main_currency_emoji()}', "candies", 50),
    (iufi.TIERS_BASE.get("rare")[0], "roll.rare", 1),
    (f'{iufi.get_main_currency_emoji()}', "candies", 100),
    (iufi.TIERS_BASE.get("epic")[0], "roll.epic", 1),
    (f'{iufi.get_main_currency_emoji()}', "candies", 500),
...
class Profile(commands.Cog):
    ...
    async def daily(self, ctx: commands.Context):
        ...
        embed.description = f"Daily reward claimed! + {f'{iufi.get_main_currency_emoji()} 5' if claimed % 5 else f'{WEEKLY_REWARDS[(claimed//5) - 1][0]} {WEEKLY_REWARDS[(claimed//5) - 1][2]}'}"
    ...
    async def inventory(self, ctx: commands.Context):
        ...
        embed.description = f"```{f'{iufi.get_main_currency_emoji()} {iufi.get_main_currency_name()}':<20} x{user['candies']}\n"
