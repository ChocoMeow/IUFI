            ...
            if item[1].split(".")[1] == selected_item.lower():
                ...
                if modal.quantity:
                    ...
                    if user["candies"] < price:
                        return await interaction.followup.send(f"You don't have enough Pumpkins! You only have `{user['candies']}` Pumpkins", ephemeral=True)
                    ...
                    func.logger.info(f"User {interaction.user.name}({interaction.user.id}) purchased {modal.quantity} {selected_item.lower()} for {price} Pumpkins.")
...
class ShopView(discord.ui.View):
    ...
    async def build_embed(self) -> discord.Embed:
        ...
        embed.description = f"🎃 Pumpkins: `{user.get('candies', 0)}`\n```"
