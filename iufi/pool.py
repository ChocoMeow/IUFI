...
class CardPool:
    ...
    @classmethod
    def _sample_available_cards(cls, cat: str, amt: int, boosted_ids: set[str] | None) -> List[Card]:
        pool = cls._available_cards[cat]
        boost = func.settings.WISHLIST_ROLL_WEIGHTS.get(cat, 1.0)

        if not boosted_ids or boost <= 1.0:
            return sample(pool, k=amt)

        if not any(card.id in boosted_ids for card in pool):
            return sample(pool, k=amt)

        remaining = list(pool)
        picked: List[Card] = []
        for _ in range(amt):
            weights = [boost if card.id in boosted_ids else 1.0 for card in remaining]
    ...
    @classmethod
    def roll(cls, amount: int = 3, *, included: List[str] = None, avoid: List[str] = None, luck_rates: float = None, soft_pity_boosts: dict = None, boosted_cards: List[str] = None) -> List[Card]:
        ...
        boosted_ids = set(boosted_cards) if boosted_cards else None
        cards = [
            card
            for cat, amt in Counter(results).items()
            for card in cls._sample_available_cards(cat, amt, boosted_ids)
        ]
        shuffle(cards)
        return cards
